...
export function buildRuntimePrompt(options: RuntimeBridgeOptions): string {
  ...
  return [
    ...
    `  - self-improving-coding-team-os.json, coding-team-metrics.json, delivery-retrospective.json, learning-feedback-loop.json, improvement-backlog.json, skill-evolution-candidates.json, and self-improvement-roadmap.md when present (delivery/quality/collaboration/learning/runtime metrics, retrospectives, feedback loops, improvement backlog, and approval-gated skill evolution candidates)`,
    `  - predictive-engineering-os.json, intent-architecture-compiler.json, change-simulation-engine.json, risk-forecast-score.json, implementation-path-ranking.json, test-intelligence-planner.json, proof-contract.json, architecture-drift-detector.json, context-freshness-gate.json, predictive-timeout-guard.json, and predictive-engineering-roadmap.md when present (predictive intent architecture, change simulation, risk forecast, implementation path ranking, test intelligence plan, proof contract, architecture drift detection, context freshness, Kimi timeout guard, and predictive roadmap)`,
    ``,
    `Follow Quest Mode + Experts Mode. Execute all tasks in the plan.`,
    `Step budget guard: treat this runtime call as one bounded Quest execution cycle. Do not exhaustively read every optional sidecar before acting. Load quest.json, plan.json, events.ndjson, current user instructions, and only the v9-v21 sidecars directly needed for the touched files, symbols, tests, runtime adapter, evidence replay, deep thinking review, idea-to-build plan, delivery retrospective, self-improvement roadmap, predictive proof/risk/timeout guard, or product-architecture recommendation.`,
    ...
    `Before execution, use interaction-memory.json, agent-memory.json, memory-graph.json, .oac/repo-wiki/, Quest v9 coding-intelligence sidecars, Coding Autopilot sidecars, Coding Execution sidecars, Verified Knowledgebase sidecars, Semantic Repo Brain sidecars, Temporal Memory sidecars, Intelligent Coding Team OS sidecars, Verified Coding Delivery OS sidecars, Product Architect Intelligence sidecars, Runtime Reliability + Evidence Replay OS sidecars, Deep Coding Collaboration OS sidecars, Self-Improving Coding Team OS sidecars, and Predictive Engineering OS sidecars to avoid repeating work, reuse known context, understand request/task/file/context relationships, think deeply from idea to build, prove delivery readiness, avoid repeating broken commands, measure team improvement, forecast risk, choose the safest implementation path, and recommend the next product/architecture/build step.`,
    ...
    `The CLI refreshes Quest v9 coding-intelligence, Coding Autopilot, Coding Execution, Verified Knowledgebase, Semantic Repo Brain, Temporal Memory, Intelligent Coding Team OS, Verified Coding Delivery OS, Product Architect Intelligence, Runtime Reliability + Evidence Replay OS, Deep Coding Collaboration OS, Self-Improving Coding Team OS, and Predictive Engineering OS sidecars after Quest creation, file/context/validation events, coding events, and review/verify/complete lifecycle changes. Run oac quest-v9 when you need a fresh coding/autopilot/execution/verified-knowledgebase/semantic/temporal/team/delivery/product-architect/runtime-reliability/deep-collaboration/self-improvement/predictive-engineering review snapshot, and run oac quest-replay <quest-id> to inspect replayable proof.`,
