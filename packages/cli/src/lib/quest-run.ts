...
export interface QuestRunArtifacts {
  ...
  productArchitectReview?: string
  architectureNextSteps?: string
  roadmapSignals?: string
  capabilityGapMap?: string
  productRiskRegister?: string
  userValueMatrix?: string
  ...
  selfImprovingCodingTeam?: string
  codingTeamMetrics?: string
  deliveryRetrospective?: string
  learningFeedbackLoop?: string
  improvementBacklog?: string
  skillEvolutionCandidates?: string
  selfImprovementRoadmap?: string
  predictiveEngineering?: string
  intentArchitectureCompiler?: string
  changeSimulationEngine?: string
  riskForecastScore?: string
  implementationPathRanking?: string
  testIntelligencePlanner?: string
  proofContract?: string
  architectureDriftDetector?: string
  contextFreshnessGate?: string
  predictiveTimeoutGuard?: string
  predictiveEngineeringRoadmap?: string
  summary?: string
  handoff?: string
}
...
export function buildQuestRun(
  ...
  const artifacts: QuestRunArtifacts = {
    ...
    productArchitectReview: 'product-architect-review.json',
    architectureNextSteps: 'architecture-next-steps.json',
    roadmapSignals: 'roadmap-signals.json',
    capabilityGapMap: 'capability-gap-map.json',
    productRiskRegister: 'product-risk-register.json',
    userValueMatrix: 'user-value-matrix.json',
    ...
    selfImprovingCodingTeam: 'self-improving-coding-team-os.json',
    codingTeamMetrics: 'coding-team-metrics.json',
    deliveryRetrospective: 'delivery-retrospective.json',
    learningFeedbackLoop: 'learning-feedback-loop.json',
    improvementBacklog: 'improvement-backlog.json',
    skillEvolutionCandidates: 'skill-evolution-candidates.json',
    selfImprovementRoadmap: 'self-improvement-roadmap.md',
    predictiveEngineering: 'predictive-engineering-os.json',
    intentArchitectureCompiler: 'intent-architecture-compiler.json',
    changeSimulationEngine: 'change-simulation-engine.json',
    riskForecastScore: 'risk-forecast-score.json',
    implementationPathRanking: 'implementation-path-ranking.json',
    testIntelligencePlanner: 'test-intelligence-planner.json',
    proofContract: 'proof-contract.json',
    architectureDriftDetector: 'architecture-drift-detector.json',
    contextFreshnessGate: 'context-freshness-gate.json',
    predictiveTimeoutGuard: 'predictive-timeout-guard.json',
    predictiveEngineeringRoadmap: 'predictive-engineering-roadmap.md',
    ...options.artifacts,
  }
...
export function normalizeQuestRun(quest: QuestRun): QuestRun {
  ...
  return {
    ...
    artifacts: {
      ...
      productArchitectReview: quest.artifacts?.productArchitectReview ?? 'product-architect-review.json',
      architectureNextSteps: quest.artifacts?.architectureNextSteps ?? 'architecture-next-steps.json',
      roadmapSignals: quest.artifacts?.roadmapSignals ?? 'roadmap-signals.json',
      capabilityGapMap: quest.artifacts?.capabilityGapMap ?? 'capability-gap-map.json',
      productRiskRegister: quest.artifacts?.productRiskRegister ?? 'product-risk-register.json',
      userValueMatrix: quest.artifacts?.userValueMatrix ?? 'user-value-matrix.json',
      ...
      selfImprovingCodingTeam: quest.artifacts?.selfImprovingCodingTeam ?? 'self-improving-coding-team-os.json',
      codingTeamMetrics: quest.artifacts?.codingTeamMetrics ?? 'coding-team-metrics.json',
      deliveryRetrospective: quest.artifacts?.deliveryRetrospective ?? 'delivery-retrospective.json',
      learningFeedbackLoop: quest.artifacts?.learningFeedbackLoop ?? 'learning-feedback-loop.json',
      improvementBacklog: quest.artifacts?.improvementBacklog ?? 'improvement-backlog.json',
      skillEvolutionCandidates: quest.artifacts?.skillEvolutionCandidates ?? 'skill-evolution-candidates.json',
      selfImprovementRoadmap: quest.artifacts?.selfImprovementRoadmap ?? 'self-improvement-roadmap.md',
      predictiveEngineering: quest.artifacts?.predictiveEngineering ?? 'predictive-engineering-os.json',
      intentArchitectureCompiler: quest.artifacts?.intentArchitectureCompiler ?? 'intent-architecture-compiler.json',
      changeSimulationEngine: quest.artifacts?.changeSimulationEngine ?? 'change-simulation-engine.json',
      riskForecastScore: quest.artifacts?.riskForecastScore ?? 'risk-forecast-score.json',
      implementationPathRanking: quest.artifacts?.implementationPathRanking ?? 'implementation-path-ranking.json',
      testIntelligencePlanner: quest.artifacts?.testIntelligencePlanner ?? 'test-intelligence-planner.json',
      proofContract: quest.artifacts?.proofContract ?? 'proof-contract.json',
      architectureDriftDetector: quest.artifacts?.architectureDriftDetector ?? 'architecture-drift-detector.json',
      contextFreshnessGate: quest.artifacts?.contextFreshnessGate ?? 'context-freshness-gate.json',
      predictiveTimeoutGuard: quest.artifacts?.predictiveTimeoutGuard ?? 'predictive-timeout-guard.json',
      predictiveEngineeringRoadmap: quest.artifacts?.predictiveEngineeringRoadmap ?? 'predictive-engineering-roadmap.md',
    },
...
export function formatRuntimeHandoff(
  ...
  lines.push('  Read Self-Improving Coding Team OS sidecars when present: self-improving-coding-team-os.json, coding-team-metrics.json, delivery-retrospective.json, learning-feedback-loop.json, improvement-backlog.json, skill-evolution-candidates.json, and self-improvement-roadmap.md.')
  lines.push('  Use Self-Improving Coding Team OS after validation/completion: measure delivery, quality, collaboration, learning, and runtime health; capture retrospectives; create approval-gated skill evolution candidates; and choose the next improvement from the backlog with user approval.')
  lines.push('  Read Predictive Engineering OS sidecars when present: predictive-engineering-os.json, intent-architecture-compiler.json, change-simulation-engine.json, risk-forecast-score.json, implementation-path-ranking.json, test-intelligence-planner.json, proof-contract.json, architecture-drift-detector.json, context-freshness-gate.json, predictive-timeout-guard.json, and predictive-engineering-roadmap.md.')
  lines.push('  Use Predictive Engineering OS before editing and before completion: compile intent architecture, simulate change impact, forecast risk, rank implementation paths, plan validation, enforce proof contracts, detect architecture drift, refresh stale context, and prevent Kimi timeout or max-step loops.')
