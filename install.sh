...
install_codex_integration() {
  ...
  if [ -f "$codex_cfg_script" ]; then
    ...
    if configure_codex_quest_default_session; then
      print_info "Codex main session configured for Quest v8 lifecycle + Quest v9-v21 intelligence stack (developer_instructions)"
    else
      print_warning "Could not set Codex Quest v8/v9-v21 default in ~/.codex/config.toml"
    fi
  fi
    ...
    print_info "Run: codex -C .  (Quest v8 lifecycle + Quest v9-v21 sidecars apply to the main session after install)"
}
