...
Runtimes append progress to `events.ndjson`; they do not rewrite `quest.json`. Use `oac quest-status` to list or inspect the reconciled run state, `oac quest-resume <quest-id>` to print OpenCode, Kimi, Claude, and Codex resume commands, `oac quest-v9 <quest-id>` to refresh coding intelligence plus the v12 Verified Knowledgebase, v13 Semantic Repo Brain, v14 Temporal Memory, v15 Intelligent Coding Team OS, v16 Verified Coding Delivery OS, v17 Product Architect Intelligence, v18 Runtime Reliability + Evidence Replay OS, v19 Deep Coding Collaboration OS, and v20 Self-Improving Coding Team OS, `oac quest-replay <quest-id>` to inspect replayable proof, and `oac runtime-doctor --runtime kimi` to check runtime health before Kimi release-ready claims. Resume does not change models; OpenAgent continues with the selected runtime model.
